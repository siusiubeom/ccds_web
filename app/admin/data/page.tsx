...
export default function AdminDataPage() {
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (res.ok) {
      setImportMsg(`Imported ${data.imported} rows. Skipped ${data.skipped}.`);
      load();
    } else {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <label className={`px-4 py-2 rounded-lg text-sm font-medium cursor-pointer border ${
            importing ? "opacity-50 cursor-not-allowed bg-gray-100" : "bg-blue-600 text-white hover:bg-blue-700"
          }`}>
      ...
      <p className="text-xs text-gray-400 mb-4">
        CSV columns: <code>cxcl10</code>, <code>nox4</code> (required) ·{" "}
        <code>rbp4</code>, <code>ccdr_ori</code>, <code>label</code> (optional)
      </p>
